...
interface ThunderPaginationProps {
  currentPage: number;
  totalPages: number;
  onPageChange: (page: number) => void;
}

const ThunderPagination: React.FC<ThunderPaginationProps> = ({ currentPage, totalPages, onPageChange }) => {
  return (
    <div className="mb-8 mt-4 flex items-center justify-center space-x-2">
      <button
        onClick={() => onPageChange(Math.max(1, currentPage - 1))}
        disabled={currentPage === 1}
        className="rounded-xl text-gray-700 transition-all duration-300 ease-in-out hover:bg-gray-300 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50">
...
export default ThunderPagination;
