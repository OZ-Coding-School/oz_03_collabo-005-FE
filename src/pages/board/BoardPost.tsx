  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="relative mx-auto max-w-full rounded-lg bg-white p-4">
          ...
          <div className="mb-2 flex items-center">
            <button
              type="button"
              className={`mr-2 h-[35px] rounded-lg px-2 ${selectedPayment === '소셜 다이닝 후기' ? 'bg-[#F5E3DB]' : 'bg-[#F2F2F2]'}`}
              onClick={() => setSelectedPayment('소셜 다이닝 후기')}>
              소셜 다이닝 후기
            </button>
            <button
              type="button"
              className={`mr-2 h-[35px] rounded-lg px-2 ${selectedPayment === '맛집 추천' ? 'bg-[#F5E3DB]' : 'bg-[#F2F2F2]'}`}
              onClick={() => setSelectedPayment('맛집 추천')}>
              맛집 추천
            </button>
          </div>
      ...
      <ModalCenter isOpen={isFormModalOpen} onClose={toggleFormModal} title1="폼 제출 테스트" title2="">
        <p className="text-center">카테고리: {selectedPayment}</p>
